...
class Builder:
  ...
  def build_lame(self, install_dir):
  ...
  def build_faac(self, install_dir):
    os.chdir("faac-1.28")
    command("./configure --prefix=%s --disable-shared --enable-static" % install_dir)
    command(["make"])
    command(["make", "install"])
  ...
  def build_x264(self, install_dir):
    os.chdir("x264")
  ...
  def build_sdl(self, install_dir):
    os.chdir("SDL-1.2.15")
  ...
  def build_ffmpeg(self, install_dir, version):
