...
class Builder:
  ...
  def build_x264(self):
    ...
    command("./configure --prefix=%s --extra-cflags=\"-I%s/include\" --extra-ldflags=\"-L%s/lib\" --enable-static --disable-lavf --disable-ffms --disable-opencl" % (self.install_dir_, self.install_dir_, self.install_dir_))
    command(["make"])
    command(["make", "install"])
    os.chdir(saved)
  ...
  def build_ffmpeg(self, version):
    ...
    command("sed -i -e 's|SDL_CONFIG=\"${cross_prefix}sdl-config\"|SDL_CONFIG=\"%s/bin/sdl-config\"|' ./configure" % self.install_dir_)
    shutil.copy2("configure", "configure.orig")
    cmd = "./configure --prefix=%s --extra-cflags=\"-I%s/include\" --extra-ldflags=\"-L%s/lib\" --enable-libfaac --enable-libmp3lame --enable-libx264 --enable-gpl --enable-pthreads --enable-nonfree" % (self.install_dir_, self.install_dir_, self.install_dir_)
    command(cmd)
